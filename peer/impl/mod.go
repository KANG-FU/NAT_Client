	...
	go func() {
		for{
			...
			if pkt.Header.Source != n.conf.Socket.GetAddress() {
				...
				err = n.socketNAT.Send(targetAddr, pkt, sendTimeout)
				if err != nil {
					fmt.Println(err.Error())
					log.Err(err).Msg("errors: failed to send unicast message")
					return
...
// 		Payload: data,
// 	}
// 	header := transport.NewHeader(n.conf.Socket.GetAddress(), n.conf.Socket.GetAddress(), serverIP, 0)
// 	pkt := transport.Packet{
// 		Header: &header,
// 		Msg: &msg,
// 	}
// 	err = n.conf.Socket.Send(serverIP, pkt, sendTimeout)
// 	if err != nil {
// 		fmt.Println(err.Error())
// 		log.Err(err).Msg("errors: failed to send unicast message")
// 		return
